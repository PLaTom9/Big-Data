...
val df = spark.read.format("csv").option("header","true").schema(schemat).load(filePath)
display(df)
...
val par = df.repartition(60)

// COMMAND ----------

// MAGIC %fs rm -r dbfs:/FileStore/tables/lab-10

// COMMAND ----------

par.write.format("delta").partitionBy("Country").mode("overwrite").save("dbfs:/FileStore/tables/lab-10/online_retail")
...
spark.sql(s"""
  ...
  USING Delta
  LOCATION 'dbfs:/FileStore/tables/lab-10/online_retail'
""")

// COMMAND ----------

val dfDelta = spark.read
  .format("delta").load("dbfs:/FileStore/tables/lab-10/online_retail")
...
// MAGIC %md
// MAGIC Różnica w czasie jest znaczna, zapytanie działa ponad 20 razy szybciej po zastosowaniu OPTIMIZE i ZORDER
...
// TODO
val plikiPrzed = dbutils.fs.ls("dbfs:/FileStore/tables/lab-10/online_retail/Country=Sweden").length
...
// TODO
val plikiPo = dbutils.fs.ls("dbfs:/FileStore/tables/lab-10/online_retail/Country=Sweden").length

// COMMAND ----------

// MAGIC %md
// MAGIC Liczba plików zmniejszyła się z 61 do 1. 
